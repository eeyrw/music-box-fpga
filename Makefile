...
test-mcu-usb-audio-rate:
	mkdir -p $(BUILD_DIR)
	cc -std=c11 -Wall -Wextra -Werror -pedantic -Imcu \
		mcu/usb_audio_rate_match.c mcu/usb_audio_rate_match_test.c \
		-o $(BUILD_DIR)/usb_audio_rate_match_test
	$(BUILD_DIR)/usb_audio_rate_match_test
...
test: check-generated test-mcu-usb-audio-rate check-docs test-ch347-python test-cpp-unit test-rtl-core test-rtl-peripheral test-sample-window test-direct-memory-model test-ddr3-model test-qspi-nor-model test-parallel-nor-model test-render-effects-harness
...
benchmark-mcu-sf2-runtime: mcu-sf2-asset
	$(CXX) $(CXX_STD_FLAGS) $(RENDER_OPT_FAST) -I. \
		host/mcu_sf2_asset_runtime.cpp \
		sim/harness/apps/mcu_sf2_runtime_benchmark_main.cpp \
		sim/harness/formats/mcu_sf2_modulation.cpp \
		sim/harness/formats/mcu_sf2_asset.cpp \
		sim/harness/formats/sf2_loader.cpp \
		sim/harness/control/command_control.cpp \
		-o $(BUILD_DIR)/mcu_sf2_runtime_benchmark
	$(BUILD_DIR)/mcu_sf2_runtime_benchmark "$(MCU_SF2_ASSET)"
...
host-realtime-midi:
	...
	$(CXX) $(CXX_STD_FLAGS) $(RENDER_OPT_FAST) -I. \
		...
		host/realtime_region_bank.cpp host/mcu_sf2_asset_runtime.cpp \
		host/command_scheduler.cpp \
