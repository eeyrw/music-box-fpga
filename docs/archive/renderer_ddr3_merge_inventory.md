...
> **Archived:** This is a completed merge record, not a current design or
> verification contract. See [`../design/system_design.md`](../design/system_design.md)
> and [`../verification/simulation_design.md`](../verification/simulation_design.md).
...
The list below records the state at merge time. The legacy source directories
and board template named here were removed later; they are now available only
through Git history.

The following were preserved in the repository even when they were outside the
production filelist:
...
At merge time these files were retained for history only. They were excluded
from current filelists, default tests, and compatibility contracts.
...
- [`../verification/simulation_design.md`](../verification/simulation_design.md):
  self-checking and render-flow details.
