...
`docs/register_map.md`, `docs/command_stream.md`, and the generated
...
The renderer accumulates saturated PCM16 voice contributions into an exact
signed 25-bit stereo block mix. `voice_major_system` sends the wide mix through
...
or phase/filter state. The effects occupy the board-facing system path between
the wide mix and the compressor:
...
- `voice_major_command_plane` length, semantic validation, and active global
  configuration;
- `voice_major_system` effect-chain configuration and dispatch;
- C++ command builders, sinks, and bit-exact reference model;
- command-plane and integrated render self-checking tests.

The command contract is retained in interface version `0x000d0000`. Exact
