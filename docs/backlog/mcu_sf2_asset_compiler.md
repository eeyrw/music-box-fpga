...
decode, static Note On materialization, filter construction, and caller-owned
17-word START packing. MT6276 and
`SGM-v2.01-NicePianosGuitarsBass-V1.2.sf2` exhaustively pass every materialized
zone/key against the C++ oracle. Voice allocation and continuing
modulation/control-tick state remain open, so the second through sixth checklist
items are not yet complete.
...
timecent, attenuation, and sustain tables; a 257-entry quarter-sine table serves
both pan and filter trigonometry through symmetry. Together they occupy 2,052
read-only bytes. A host `-Os -ffreestanding -fno-builtin` build currently
reports 7,821 total text plus read-only bytes, zero data, and zero BSS before
