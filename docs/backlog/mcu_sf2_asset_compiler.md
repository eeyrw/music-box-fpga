...
zone/key against the C++ oracle. The runtime trace covers
Note On, controller and pressure changes, pitch bend, four control ticks,
sustain release, voice reuse, and bounded stealing against the C++ runtime on
both MT6276 and SGM. The SGM trace explicitly selects bank 0, program 0, key 60
and compares the complete initial START payload, including modulation-folded
phase, gain, filter, and envelope fields. Runtime gain, pitch, and filter fields
remain within the one-LSB numeric contract. Production-MCU cross-compilation,
resource reports, and target timing remain open, so the final checklist item is
not yet complete.
