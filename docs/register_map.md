...
| `0x908c` | EVENT_FIFO_DATA3 | envelope event payload2 word |
| `0x9090` | EVENT_FIFO_PUSH | write one to push the assembled event |
...
An envelope event is assembled in four 32-bit data registers and pushed by
...
EVENT_FIFO_DATA3[31:0]  = payload2
```
...
| `2` | `EVT_VOL_ATTACK` | `payload0 = target_q15`, `payload1[23:0] = duration_samples`; linear ramp from zero. |
| `3` | `EVT_VOL_DECAY_CB` | `payload0 = start_cb`, `payload1[15:0] = target_cb`, `payload2[23:0] = duration_samples`; integer cB endpoints are converted to Q8.8 internally. |
| `4` | `EVT_VOL_RELEASE_CB` | `payload0 = start_cb`, `payload2[23:0] = duration_samples`; output reaches zero at 960 cB or above. |
...
`duration_samples` is an unsigned 24-bit sample count; zero is treated as one
sample. At 48 kHz the maximum single envelope stage is about 349.5 seconds.
Software clamps SoundFont timecents and modulators into this sample-domain
duration before writing primitive events.
...
`tools/gen_envelope_lut.py`. The event engine advances decay and release with a
Q0.32 phase accumulator, so long stages are not limited by a minimum cB-per-sample
step.
...
On real SPI links, event writes take nonzero time. Software must not enable a
voice and then rely on later envelope events arriving before the renderer scans
that voice. If the voice commit wins the race, the first samples use the committed
or fallback runtime envelope level; this can be silence if the initial envelope
is zero, or a short stale-level leak if the voice was reused before the generated
state was reset. If an event reaches the FIFO after its timestamp, the engine
sets `late`; if a same-timestamp event arrives after the renderer has already
scanned that voice, it sets `order_error` and the event is delayed until a later
matching snapshot.

The hardware-safe Note On sequence is:

```text
read EVENT_TIME
T = EVENT_TIME + safety_offset_samples
push EVT_ENV_SET 0 and the remaining Note On envelope events timestamped at T
write voice shadow/runtime configuration
write VOICE_CONTROL.enable|apply
```

The first Note On event for a reused voice must be `EVT_ENV_SET`, normally with
`payload0 = 0`, so any previous generated envelope state is overwritten before
the voice becomes audible. `safety_offset_samples` must cover the worst-case SPI
burst time for the event and voice-register writes plus at least one renderer
voice-scan margin. At 48 kHz, 1 ms is 48 samples and 5 ms is 240 samples. Pure
simulation may use zero offset because event writes and commits are scheduled
without physical SPI latency; board software should use a positive offset.
