module tb_smart_artix_ddr3_reg_access_master;
  logic clk;
  logic rst;
  logic start;
  logic write;
  ...
  smart_artix_ddr3_reg_access_master dut (
    .clk,
    .rst,
    .start,
    .write,
...
/* verilator lint_on BLKSEQ */
  ...
  initial begin
    ...
    start = 1'b0;
    #1;
    check(error_pulse, "unaligned reg access did not report error");
    check(ready && !busy, "unaligned reg access should remain idle");
    ...
    check(mig_app_write_data.data == wdata, "reg write data mismatch");
    check(mig_app_write_data.mask == 16'hff00, "reg write mask mismatch");
    @(negedge clk);
    #1;
    check(done_pulse, "reg write did not complete after command/data ready");
    check(ready && !busy, "reg write did not return to idle");
    ...
    $display("PASS: smart_artix_ddr3_reg_access_master");
    $finish;
  end
endmodule
