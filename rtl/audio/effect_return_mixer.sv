module effect_return_mixer (
  ...
  logic effect_mix_saturated_l;
  logic effect_mix_saturated_r;
  logic [2:0] saturation_events;
  ...
  function automatic logic [31:0] sat_add(
    ...
    if (32'(amount) > (32'hffff_ffff - value))
  ...
  always_comb begin
    ...
    reverb_input_wide_l = $signed({{2{reverb_dry_scaled_l[47]}},
                                   reverb_dry_scaled_l}) +
                          $signed({{2{chorus_route_scaled_l[47]}},
                                   chorus_route_scaled_l});
    reverb_input_wide_r = $signed({{2{reverb_dry_scaled_r[47]}},
                                   reverb_dry_scaled_r}) +
                          $signed({{2{chorus_route_scaled_r[47]}},
                                   chorus_route_scaled_r});
    ...
    effect_mix_saturated_l = (effect_mix_wide_l > 50'sd8388607) ||
                             (effect_mix_wide_l < -50'sd8388608);
    effect_mix_saturated_r = (effect_mix_wide_r > 50'sd8388607) ||
                             (effect_mix_wide_r < -50'sd8388608);
    saturation_events = 3'd0;
    if (reverb_input_commit_i)
      saturation_events = saturation_events +
                          {2'd0, reverb_input_saturated_l_o} +
                          {2'd0, reverb_input_saturated_r_o};
    if (in_valid && in_ready)
      saturation_events = saturation_events +
                          {2'd0, effect_mix_saturated_l} +
                          {2'd0, effect_mix_saturated_r};
  end
  ...
  always_ff @(posedge clk) begin
    if (rst || clear_i) begin
      ...
      if (in_valid && in_ready) begin
        out_l <= saturate_mix(effect_mix_wide_l);
        out_r <= saturate_mix(effect_mix_wide_r);
        out_valid <= 1'b1;
      end else if (out_valid && out_ready) begin
        out_valid <= 1'b0;
      end
      if (saturation_events != 0)
        saturation_count <= sat_add(saturation_count, saturation_events);
      if ((reverb_input_commit_i &&
