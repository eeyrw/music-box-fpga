module voice_line_cache #(
  ...
  logic [NUM_VOICES-1:0][LINES_PER_VOICE-1:0] cache_prefetched;
  ...
  always_comb begin
    ...
    prefetch_fill_way = replace_way[req.voice];
    prefetch_fill_replaces_valid = cache_valid[req.voice][replace_way[req.voice]];
    prefetch_fill_replaces_prefetched = cache_prefetched[req.voice][replace_way[req.voice]];
    ...
    for (int w = 0; w < LINES_PER_VOICE; w++) begin
      ...
      if (cache_valid[req.voice][w] && cache_tag[req.voice][w] == next_prefetch_tag)
  ...
  always_ff @(posedge clk) begin
    if (rst) begin
      ...
      if ((state == STATE_IDLE) && prefetch_inflight && ext_rsp_valid && !same_cycle_prefetch_demand) begin
        prefetch_inflight <= 1'b0;
        cache_valid[prefetch_voice][prefetch_way] <= 1'b1;
        cache_prefetched[prefetch_voice][prefetch_way] <= 1'b1;
        cache_tag[prefetch_voice][prefetch_way] <= prefetch_tag;
        cache_line[prefetch_voice][prefetch_way] <= ext_rsp_data;
        replace_way[prefetch_voice] <=
        ...
        STATE_IDLE: begin
          if (req.valid) begin
            ...
            if (hit) begin
              ...
              if (cache_prefetched[req.voice][hit_way]) begin
                cache_prefetched[req.voice][hit_way] <= 1'b0;
              ...
              if (same_cycle_prefetch_demand) begin
                cache_valid[prefetch_voice][prefetch_way] <= 1'b1;
                cache_prefetched[prefetch_voice][prefetch_way] <= 1'b0;
                cache_tag[prefetch_voice][prefetch_way] <= prefetch_tag;
                cache_line[prefetch_voice][prefetch_way] <= ext_rsp_data;
                replace_way[prefetch_voice] <=
                  (prefetch_way == WAY_WIDTH'(LINES_PER_VOICE - 1)) ? '0 : prefetch_way + 1'b1;
